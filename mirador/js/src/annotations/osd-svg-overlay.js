(function($) {
  ...
  $.Overlay.prototype = {
    ...
    replaceShape: function(shape, annotation) {
      ...
      var centroid_x = 0;
      var centroid_y = 0;
      for(i=0;i<shape.segments.length;i++){
        centroid_x+=shape.segments[i].point.x;
        centroid_y+=shape.segments[i].point.y;
      }
      centroid_x = centroid_x/shape.segments.length;
      centroid_y = centroid_y/shape.segments.length;
      cloned.data.centroid={x: centroid_x, y: centroid_y};

      var annoTextList = [];
      for(i=0; i<annotation.resource.length; i++) {
        var anno = annotation.resource[i];
        if (anno["@type"]=="oa:Tag") {
          annoTextList.push(anno.chars);
        }
      }

      cloned.data.annoText = annoTextList.join(" ");

      cloned.data.defaultStrokeValue = 10;
